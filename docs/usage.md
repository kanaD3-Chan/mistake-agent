...
- **设置**：左侧导航「设置」配置主模型/视觉模型的 URL、Key、模型 ID、接入方式与日志级别；「英语练习模式」开启后，模型对话、判分、出题与复盘统一使用英文，界面文字保持中文；Key 只显示"已设置"状态，留空表示保留原 Key。设置页还有「教学规则（AGENTS.md）」卡片：展示规则是否已加载（缺失/损坏/超限会回退默认规则），可一键用系统默认程序打开规则文件编辑，保存后即时生效；英文模式下中文规则照常加载，由模型理解后用英文输出。

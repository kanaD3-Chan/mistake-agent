...
本决策 supersede 架构红线「单 crate：src/kernel/ 与 src/plugin/ 分区，不新增 crate 拆分」（AGENTS.md / PROJECT.md §4）——该红线服务于 v2 单应用的简单性；跨应用复用的目标使拆分成为必要。状态：M1-M4 已落地（2026-08-11），M5 待办；实施顺序见 [docs/plan/so-lite-agent.md](../plan/so-lite-agent.md)。
